...
procedure example03 is
   ...
   solution : Complex_Matrix (1..4,1..2);
...
begin
   ...
   solution := SolveSystem (matrix,rhs);
          
   Put_line ("Solution");
   for i in solution'range(1) loop
      for j in solution'range(2) loop
         put(" ");
         put(solution(i,j),3,3,0);
      end loop;
      new_line;
   end loop;
